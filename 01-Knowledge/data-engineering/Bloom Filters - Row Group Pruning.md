...
subdomain: parquet-query-optimization
note_type: adr
source_type: self
status: evergreen
level: advanced
---
# AI Summary
Architecture Decision Record documenting the adoption of Bloom Filters for row-group pruning in Parquet-based data lakes. The decision addresses inefficient equality filtering on high-cardinality columns by layering Bloom Filters after partition, file, and min/max pruning. It specifies a 1% false-positive target, selection criteria for eligible columns, expected query performance improvements, implementation trade-offs, operational costs, and alternative approaches such as hash sets, bitmap indexes, and Count-Min Sketches. The ADR provides both technical justification and long-term architectural rationale for improving storage efficiency and query performance.
...
- [[Bloom Filters]] — conceptual deep dive: internals, FP-rate math
- [[Parquet]] — storage format the filter sits in
- [[Lakehouse Performance Optimization]] — where pruning sits in the query path
- [[Partitioning]] — the pruning layer above this
- [[_Data Engineering MOC]] — domain index
