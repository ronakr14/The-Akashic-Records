...
---
# AI Summary
A practical guide to organizing Python code using modules and packages. The note explains module creation and importing, package structure with `__init__.py`, absolute versus relative imports, project organization best practices, dynamic imports, module reloading, Python path management, package inspection utilities, API exposure through `__all__`, and techniques for avoiding circular imports. It concludes with a concise cheat sheet that summarizes recommended patterns for building maintainable and well-structured Python projects.
...
Or set `PYTHONPATH` environment variable. Avoid modifying `sys.path` in production if possible.
...
- [[Python Import Path Setup]] — runtime sys.path snippet
- [[Python External Libraries Playbook]] — consuming packages across projects
- [[Python Environment Playbook]] — virtualenv and tooling
