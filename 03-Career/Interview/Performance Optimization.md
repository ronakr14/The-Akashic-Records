...
---
# AI Summary
An interview-oriented reference covering Spark batch performance optimization and troubleshooting. The note explains systematic investigation of slow jobs, optimization strategies for billion-row workloads, identification of CPU, memory, network, disk, and shuffle bottlenecks, and the importance of predicate pushdown. It emphasizes practical techniques such as broadcast joins, adaptive query execution (AQE), partition pruning, skew mitigation, join ordering, pre-aggregation, file compaction, and Spark UI analysis to diagnose and improve large-scale data processing performance.
